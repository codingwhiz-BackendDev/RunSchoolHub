...
def view_chats(request):    
    ...
    chat_friends = Chat.objects.filter(follower=request.user.username).order_by('last_message_time')
...
def check_notifications(request,pk): 
    notification = Notifications.objects.get(id=pk)
     
    return render(request, 'check_notifications.html', {'notification':notification})
